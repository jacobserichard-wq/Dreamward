...
- **Header overflow at 320px viewports.** Sub-session 12's settings-link addition (commit `f81889f`) pushed right-side header content to ~220px wide; FlowWork wordmark may overflow at 320px. Flagged in `commit12.3-report.md` §6; not blocking — Phase 2 mobile sweep is generally complete and modern devices are wider. Cheap mitigations: smaller mobile-only plan badge, or stack the right-side row beneath the wordmark at `<640px`. ~15 min commit when prioritized.
- **`processed_items` schema telemetry** (deferred from sub-session 11's audit plan; future sub-session 13 polish batch, audit commit #5). Adds `ai_classified_at`, `ai_model`, `user_corrected_category`, optional `ai_confidence` columns to `processed_items`. Enables auditing what was classified by what/when and tracking user corrections for future learning loop. ~30 min commit including Railway migration.
- **`/api/reclassify` endpoint + dashboard button** (deferred from sub-session 11's audit plan; future sub-session 13 polish batch, audit commit #6). Lets users (or admin) re-run classification on existing items with the updated category lists. Closes the "mixed-state dashboard" finding from commit `41e620c` where older items still show umbrella values. ~45–60 min commit.
- **Weekly reclassify sweep cron** (deferred from sub-session 11's audit plan; future sub-session 13 polish batch, audit commit #7). Optional. Extends `app/api/cron/route.ts` to auto-reclassify items older than N days where confidence is below a threshold. ~30 min commit. Deferred pending production traffic data on classification quality.
...
**Other Phase 2 items:**
- **AI auto-classification** — MVP shipped sub-session 11 (industry-aware classification on both Gmail and CSV ingest paths + customer-facing settings UI). Sub-session 11 testing-findings cluster closed in sub-session 12 (telecom/utilities universals, default landing tab, settings nav link, dirty-state save UX). Three follow-up commits deferred to **sub-session 13 polish batch**: schema telemetry, `/api/reclassify` endpoint + dashboard button, weekly reclassify cron. See Phase 2 loose-ends list above for full detail.
...
**Recommended next coding session:** With the sub-session 11 testing-findings cluster now closed in sub-session 12, **Calendly webhook** (Phase 1.7 Tier 2, 2–4 hours) is the primary fresh-energy candidate — turns "user visited welcome-pro" into "user actually booked," with banner copy upgrading from generic prompt to specific call time. The strong alternative is the **sub-session 13 polish batch** of the three audit-deferred AI classification commits (schema telemetry ~30 min, `/api/reclassify` endpoint + dashboard button ~45–60 min, optional weekly reclassify cron ~30 min) — together ~2 hours and the natural continuation of sub-session 11's AI capstone work. Smaller warm-ups still queued — Phase 1.7 Tier 2 items (**sample data parity** for the 7 missing industries and **Pro reminder cron** chasing unbooked Pro signups, ~1–2 hours each), plus three audit-deferred items from sub-session 10 (**README rewrite** ~30–60 min, **`.env.example` creation** ~20 min, **dependency hygiene sweep** ~30–90 min), plus the newly-logged **header overflow at 320px** (~15 min) from sub-session 12. Calendly is the headline product-trajectory move; the sub-session 13 polish batch is the alternative that closes the AI capstone fully.
