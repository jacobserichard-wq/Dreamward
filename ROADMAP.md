...
**Recommended next coding session:** The headline choice is **AI auto-classification** — the Phase 2 capstone feature (scan inbox, auto-label emails, Pro-tier-only given Gmail dependency). Bigger session, deserves its own audit-first pass; 2+ hours. The non-Phase-2 alternative remains **Calendly webhook** (Phase 1.7 Tier 2, 2–4 hours) — turns "user visited welcome-pro" into "user actually booked," with banner copy upgrading from generic prompt to specific call time. Smaller warm-ups available if appetite is lighter — Phase 1.7 Tier 2 items remain (**sample data parity** for the 7 missing industries and **Pro reminder cron** chasing unbooked Pro signups, ~1–2 hours each) and three new audit-deferred items from sub-session 10 join the list: **README rewrite** replacing the create-next-app boilerplate (~30–60 min), **`.env.example` creation** documenting the 12 distinct `process.env.*` references (~20 min), and a **dependency hygiene sweep** batching 4 in-range patch/minor updates plus a few pinning/major decisions (~30–90 min). The AI-vs-Calendly choice is what actually matters for product trajectory; the smaller items are quick wins to slot in around it.
