...
**8 Phases** · **55+ Tasks** · **17 Weeks** · **Phase 0–1 SHIPPED** · **Phase 1.5 SHIPPED** · **Phase 1.6 IN PROGRESS (OAuth submission paused — CASA decision)** · **Phase 1.7: Tier 1 + Calendly webhook shipped; Tier 2 remainder pending** · **Phase 2: mobile-responsive 10/10 + AI auto-classification (MVP, testing cluster, polish batch) shipped**
...
- **`ProcessedItem.category` type is stale.** Still typed as the narrow umbrella union (`"invoice" | "expense" | "ar_followup"`) in `app/page.tsx` despite sub-session 11 expanding the value space to industry-aware categories. Worked around in commit `727e6a9` with a `readonly string[]` cast on the umbrella-values array. Real fix is widening the type to `string` (or a wider union derived from `lib/categories`). ~10 min commit. Reference: `commit13.3-report.md` pre-flight finding §1.
- **`INDUSTRY_DISPLAY_NAMES` triplicated.** Same `Record<Industry, string>` map now exists in three places: `app/api/process/route.ts`, `app/api/upload/route.ts`, and `lib/reclassify.ts` (post-13.4 extraction). Three-site duplication is the canonical threshold where extraction earns itself — extract to a shared module (e.g., `lib/industryDisplayNames.ts`) and have all three consumers import. ~10 min commit. Reference: `commit13.3-report.md` + `commit13.4-report.md` deferred-consolidation notes.
- **`welcome_pro_seen` is now dead-write data.** Sub-session 14's three-state banner gate moved from `welcome_pro_seen` to `pro_call_booked_at` (visiting ≠ booking). The `welcome_pro_seen` field is still POSTed by `/api/welcome-pro/seen` on every welcome-pro visit and still exposed by `/api/client`, but no frontend reads it anymore. Cleanup: remove the write path (`/api/welcome-pro/seen` route + the fire-and-forget POST in `app/welcome-pro/page.tsx`), drop the field from `/api/client`, optionally a `0003_` migration to drop the column. ~10 min commit when prioritized. Reference: `commit14.3-report.md` §4.
...
1. ~~**Calendly webhook**~~ — ✅ shipped in sub-session 14 (commits `defc0a5`, `c72eae6`, `9f47eb3`). Requires two user setup steps before production traffic works — see `commit14.2-report.md`.
...
**Recommended next coding session:** Both big near-term features are now shipped — Phase 2 (AI auto-classification) and Calendly webhook. **No single "big session" remains in the near-term queue.** The honest framing of what's next:

- **Phase 1.7 Tier 2 remainder** — **sample data parity** for the 7 missing industries (~1–2 hr) and **Pro reminder cron** chasing unbooked Pro signups (~1–2 hr). These are the larger remaining items and the natural follow-ups to Calendly now that bookings are tracked.
- **Phase 1.7 Tier 3** — admin tooling for call completion + the "priority support SLA" Option A/B decision. Strategic, bigger; needs product judgment before code.
- **Accumulated small cleanups** — **`ProcessedItem.category` type widening** (~10 min), **`INDUSTRY_DISPLAY_NAMES` consolidation** (~10 min), **`welcome_pro_seen` dead-data cleanup** (~10 min). ~30 min batched together as a small follow-ups session.
- **Sub-session 10 audit follow-ups** — **README rewrite** (~30–60 min), **`.env.example` creation** (~20 min), **dependency hygiene sweep** (~30–90 min). Could batch as a documentation/hygiene session.
...
For continuation of recent momentum, the **Phase 1.7 Tier 2 remainder + accumulated small cleanups** is the most natural next session — both items closing real near-term debt. For a fresh-energy "build something new" session, **Phase 3 (Events module)** is the right size.
