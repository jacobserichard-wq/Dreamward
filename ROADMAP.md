...
**8 Phases** · **55+ Tasks** · **17 Weeks** · **Phase 0–1 SHIPPED** · **Phase 1.5 SHIPPED** · **Phase 1.6 IN PROGRESS (OAuth submission paused — CASA decision)** · **Phase 1.7 SHIPPED (white-glove Tier 1)** · **Phase 2: mobile-responsive 10/10 shipped, AI auto-classification pending**
...
- **`statusStyle()` helper still returns inline styles, not Tailwind class strings.** Current shape: `statusStyle(status)` returns `React.CSSProperties` keyed on status name. Audit recommended translation to `statusBadgeClasses(status)` returning Tailwind class strings (parallel to `lib/planColor.ts`). Will be done as part of the dashboard migration commit, not as a separate refactor — the helper is small and only has one consumer.
- **`PlanName` type collision between `lib/plans.ts` and `lib/stripe.ts`.** Two files now export `PlanName` with different membership: `lib/plans.ts` has the canonical 5-member version (`'trial' | 'starter' | 'growth' | 'pro' | 'canceled'`); `lib/stripe.ts` added a narrower 3-member version (`'starter' | 'growth' | 'pro'`) for `planFromPriceId`'s return type during sub-session 8. No runtime collision today — no consumer imports `PlanName` from either file. Latent footgun: future code importing from `@/lib/stripe` thinking it's canonical will silently get the narrower union and produce confusing TS errors elsewhere. Recommended fix: consolidate. Have `lib/stripe.ts` import `PlanName` from `lib/plans.ts` and rename the local return type to something explicit like `PaidPlanName` or just use a literal union inline. Small commit, ~10 min.
- **Dead `PLANS.features` field in `lib/stripe.ts`.** Audited and confirmed unused: `PLANS` is consumed only by `app/api/stripe/checkout/route.ts`, which reads `priceId` and never `features`. All customer-facing copy lives in `app/billing/page.tsx`'s separate `PLAN_DETAILS`. Sub-session 8 left `PLANS.features` untouched to keep commits 2 and 2.5 scoped to customer-facing edits. Recommended follow-up: delete the entire `features` field from `PLANS` rather than synchronizing it. Single commit, ~5 min.
- **16 stale `.gitignore` patterns from `776d045` now match nothing on disk.** Sub-session 8's Phase A deleted the debris these patterns were defensively covering, so the patterns are now redundant. Recommended follow-up: delete lines 46–67 of `.gitignore` (the "Pager debris" block + `*600*`); optionally relocate `.claude/` to a less buried spot in the file while there. ~5-min cleanup, zero behavior change.
...
**Recommended next coding session:** The headline choice is **AI auto-classification** — the Phase 2 capstone feature (scan inbox, auto-label emails, Pro-tier-only given Gmail dependency). Bigger session, deserves its own audit-first pass; 2+ hours. The non-Phase-2 alternative remains **Calendly webhook** (Phase 1.7 Tier 2, 2–4 hours) — turns "user visited welcome-pro" into "user actually booked," with banner copy upgrading from generic prompt to specific call time. Optional warm-up: the three new follow-ups from sub-session 8 — `PlanName` consolidation (~10 min), dead `PLANS.features` deletion (~5 min), stale gitignore pattern cleanup (~5 min) — batch into a "sub-session 8.5" of ~20 minutes total if the appetite is there. The bigger AI-vs-Calendly choice is what actually matters for product trajectory; the warm-up batch is just sitting on disk if you want a quick win first.
