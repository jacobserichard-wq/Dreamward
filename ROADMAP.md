# Dreamward Product Roadmap

**Small Business Edition** | Updated June 10, 2026

> **Renamed June 10, 2026: FlowWork → Dreamward** (domain `flowworks.it.com` → `godreamward.com`). Historical entries below were bulk-renamed with the product; commit hashes and quoted URLs are preserved as written. Internal identifiers (Stripe `flowwork_*` metadata keys, the Calendly `utm_source=flowwork` tag, the wix-bridge `flowworkUrl` variable) deliberately keep the old name — they're invisible to users, and renaming them risks breaking in-flight Stripe sessions.

**9 Phases** · **60+ Tasks** · **18 Weeks** · **Phases 0 / 1 / 1.5 / 2 / 3 / 4 / 5 / 6 / 6.5 / 7 / 8 SHIPPED** · **Phase 1.6 IN PROGRESS (OAuth submission paused — CASA decision)** · **Phase 1.7: Tier 1 + Tier 2 complete; Tier 3 (admin tooling) pending** · **Phase 8 SHIPPED (Shopify integration — OAuth + token encryption + connect/disconnect UI + backfill with $99 unlimited upgrade + webhooks + cron + landing page command-center pivot)** · **Phase 9.1 SHIPPED (Channel Profitability — canonical 9-channel dashboard surface + /profitability "By Channel" tab + per-user collapse persistence + overhead allocation toggle, repositions Dreamward from "bookkeeping software" to "decision-support command center")**

---

## UX Principles

Dreamward uses standard accounting vocabulary (AR aging, expense categories, mileage, Schedule C). Our customers include bookkeepers and small CPA firms who expect that language. But the *experience* is built for someone who has never used accounting software before.
...
- [x] **Custom domain `godreamward.com` connected** — Vercel A record (216.198.79.1), DNS propagated, SSL provisioned. Both apex and `www.godreamward.com` (via redirect) live. *Existing Resend TXT records preserved.* — *infra*
- [x] **NextAuth + Google OAuth migrated to custom domain** — `NEXTAUTH_URL` updated, Google Cloud OAuth client now lists `godreamward.com` in both Authorized JavaScript origins and Authorized redirect URIs. Old vercel.app entries kept for transition. — *infra*
- [x] **Privacy policy live** at `godreamward.com/privacy` — covers Google Limited Use commitment, subprocessor transparency, deletion mechanism, AI processing disclosure. Honest disclosure of `gmail.readonly` scope grant vs. application-level limited use (added May 5 after audit). — *legal*
- [x] **Terms of service live** at `godreamward.com/terms` — 14-day money-back guarantee, "not an accounting firm" disclaimer, Indiana governing law, $100 / 12-month liability cap. — *legal*
...
After surfacing the CASA blocker, the founder pivoted from "submit OAuth Production now" to a tiered auth strategy that matches Dreamward's actual customer mix and de-risks the CASA spend with real revenue data.
...
- [x] **Calendly URL prefilled with client identity** — `email`, `name`, `utm_source=flowwork`, `utm_content=<client_id>`. Eventual webhook will use `utm_content` to correlate bookings to Dreamward clients. — *feature*, commit `1e3f42d`
...
- **README rewrite.** Currently unchanged create-next-app boilerplate. Should describe Dreamward (AI accounting automation for small business), tech stack (Next.js 16, Tailwind v4, Postgres on Railway), and the current Phase 2 state. ~30–60 min.
...
- [x] Quarterly estimates helper — suggest estimated tax payments — *feature* — **shipped in Phase 7c (sub-session 24)**
- [x] Schedule C line-item mapping (which expenses go on which line) — *feature* — **shipped in Phase 7c (sub-session 24)**
...
**Phase 7b (sub-session 22) shipped**: PDF export of the annual summary via `@react-pdf/renderer`. `/api/reports/annual/pdf` download endpoint + Send-to-CPA now attaches both CSV and PDF. `lib/pdf/annual.tsx` renders a single-page Letter document with header, summary, two-column by-category, AR snapshot, and a Notes section that mirrors the on-screen honesty disclaimers.

**Phase 7c (sub-session 24) shipped**: every default expense category now carries a `scheduleC` field mapping it to the correct IRS Form 1040 Schedule C Part II line (8, 9, 10, 11, 13, 15, 17, 18, 20a/b, 22, 23, 24a/b, 25, 26, 27a, 30). The annual summary now exposes a `scheduleCSummary` rollup grouped by line, plus a `quarterlyEstimate` block computed by `lib/quarterly.ts` (linear YTD projection at the configured bracket — default 22% income + 14.13% SE = 36.13% effective). `/reports` gains a Quarterly Estimates panel (YTD set-aside, projected annual tax, per-quarter suggested payment, all four 1040-ES deadlines with the next one marked) and a Schedule C Summary panel; expense rows in the by-category table get L\<line\> badges. CSV gains a Schedule C Line column and a ScheduleC Summary section. PDF gains both surfaces too. Settings gets a Tax Bracket Assumption editor for the user's marginal bracket. Every Schedule C + quarterly surface carries the mandatory "not tax advice — verify with your CPA" disclaimer.

---

# What Dreamward Can Do
...
**UX First-Run polish (sub-session 24) shipped**: 11-commit arc addressing the friction a brand-new Pro user hits on the dashboard. Audit + design in `session-notes/audit-ux-firstrun.md` + `session-notes/ux-firstrun-design.md`. Highlights: new shared `ConfirmModal` (replaces `window.confirm` calls); extracted `StatCard` with `tooltip` prop + per-card explainer copy; removed the duplicate Overdue stat card (was in both stat row + Status Breakdown); Status Breakdown pills now `<button>`s that navigate to a filtered Processed view; pale-yellow tint on the dashboard tab when sample data is present; deleted-count surfaced in the clear-sample toast; new `SetupChecklist` component with up to 7 plan-gated setup steps (Connect Gmail / Upload first / Home address / Clear sample / CPA email / Tax bracket / Onboarding call), each item's done-state derived from runtime data, dismissal persists to `preferences.ux.checklist_dismissed_at`; Welcome-to-Pro + Sample-data banners suppress when the checklist surfaces the same steps; empty-state dashboard replaced with a 3-path CTA card (Connect Gmail / Upload file / Add manually); first-visit auto-scroll anchors the checklist in the viewport on first paint. No migration — uses the existing `client_settings.preferences` JSONB merge path.

**Gmail label setup guide (sub-session 24 follow-up) shipped**: 6-commit arc closing the documentation gap where the SetupChecklist's "Connect Gmail" step took users through OAuth but never told them they needed three specific Gmail labels (Invoices / AR Follow Up / Expenses) before fetches would return anything. Tightened the Gmail Pro-gate at the same time — previously `/api/gmail` + `/api/process` were callable by any signed-in user despite the README marketing Gmail as a Pro-only feature; now both routes 403 for non-Pro callers and `lib/plans.ts` drops `labels[]` from trial/starter/growth. UI side: Emails tab shows a "Pro required, $89/mo" upgrade card for non-Pro users (with a "Preview the setup guide" link); SetupChecklist Gmail item visibleOn `["pro"]` only; empty-state dashboard CTA hides the Connect-Gmail card for non-Pro. New `/help/gmail-setup` page: 4-section guide (create the labels with exact-name guidance, get emails into the right label via manual labeling vs Gmail filters, pull labeled emails into Dreamward, troubleshooting with 4 expandable items). SetupChecklist "What labels do I need?" link beneath the Connect button; Emails tab "Don't see emails when you click a label?" hint above the pill row. Help page is plan-gate previewable — non-Pro users see a "Pro required" banner at the top but can read the workflow to evaluate before upgrading.

**Flow redesign (sub-session 24 follow-up) shipped**: 7-commit arc rewriting the entry funnel — public marketing landing at `/`, tier-aware onboarding checklist at `/onboarding`, dashboard moved to `/dashboard`. Audit + design in `session-notes/audit-flow-redesign.md` + `flow-redesign-design.md` with 12 §1 decisions locked to recommended defaults. Routing: `app/page.tsx` (1769-line dashboard) moved to `app/dashboard/page.tsx`; new server-component marketing landing at `/` (hero + 4-card features + 4-tile pricing with Pro highlighted "Most popular" + footer CTA + privacy/terms links); `proxy.ts` matcher updated; 14 internal `/` links rewritten to `/dashboard`; authenticated visitors to `/` get a server-side redirect to `/dashboard` (bookmarks keep working). New `SignInButton` client island handles `signIn("google", { callbackUrl: "/onboarding" })`. SetupChecklist extended with `mode: "dashboard" | "onboarding"` prop, per-item Skip (with ConfirmModal — locked decision #4 = skip permanently), "Show N skipped items" expander, white-glove highlighted section (violet-to-fuchsia gradient, "FREE — included with Pro" pill, locked decision #7), new items (`tell_us_about_business` with inline industry-grid + business-name form, `add_first_event`, `add_first_invoice`), "All set!" CTA card when every visible onboarding item is done. `/onboarding` fully rewritten (-184 lines / +306 lines) as the canonical setup surface — no more returning-user guard, users can revisit anytime via the new "Setup" link in the dashboard nav. The legacy 2-step Welcome+Industry form is gone; industry+name now live as the inline `tell_us_about_business` item (non-skippable per locked decision #6 — industry drives AI categorization). SetupChecklist removed from dashboard (locked decision #3); banner suppression gates dropped (Welcome-to-Pro + Sample-data banners unconditional again). Skip state persists to `preferences.ux.checklist_items_skipped[itemId]` ISO timestamps via the existing PATCH /api/settings merge — no migration. All pricing CTAs route through Trial-first signup (locked decision #5); upgrade flow at `/billing` preserved unchanged.

**Phase 9.1 (Channel Profitability) shipped**: First Phase 9 BI feature. New aggregation layer (`lib/profitability/channels.ts`) maps source/category/event-id → 9 canonical channels (Shopify, Markets, Wholesale, Service work, Gmail invoices, Uploads + Etsy/Square/WooCommerce as coming-soon placeholders). GET `/api/profitability/channels?year=&mode=` returns per-channel revenue/expenses/net + unallocated overhead pool. `ChannelTable` component (horizontal full-detail variant) lives on /profitability's new "By Channel" tab with year picker + attributable-vs-allocated mode toggle.

**Phase 9.2 (Dashboard pivot to command-center layout) shipped**: Major dashboard restructure per Jacob's "command center" positioning. REMOVED: 4-card stat row (vanity metrics), Status Breakdown row, Accounts Receivable card, Top Categories grid. ADDED: `SalesBanner` (3 big numbers — Total Sales / Total Expenses / Net Profit), `ActionItemsStrip` (pill row below top nav surfacing Needs Review + Overdue with click-through filters), `ChannelStack` (vertical channel cards, default visible = Shopify + Markets only, user restores others via "Add another channel" expander), `UpcomingEventsCard` (Events filtered by start_date >= today + Promotions "Coming soon" placeholder). 2-column grid (Channels left, Events right). Marketing landing reframed in the same arc: hero sub-line shifted from tax-time framing to decision-support ("One dashboard. Every revenue source. Smarter business decisions.").
...
QuickBooks $38–$275/mo (raised prices 15–25% May 2026). Xero $25–$90/mo. FreshBooks $19–$60/mo. Wave: free (fees on payments). Dreamward undercuts all paid competitors while offering AI-first automation none of them match. All tiers include a 14-day free trial.
