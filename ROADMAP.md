...
- **Repo-root debris from earlier in the May 10 session.** Untracked files at the repo root left behind by the modal extraction setup. Modal extraction commit was scoped to specific files via `git add` so debris didn't get swept in. Quick `git status --short` and either delete or `.gitignore` whatever's there before next coding session.
...
**Recommended next coding session:** Either **AI auto-classification** — the Phase 2 capstone feature (scan inbox, auto-label emails, will end up Pro-tier-only given Gmail dependency). Bigger session, deserves its own audit-first pass. Or the **loose-ends batch** — `planFromPriceId` extraction (~20 min, code-quality cleanup), pricing copy "1 Gmail account" decision, and repo-root debris sweep — three quick wins that batch into a single ~45-minute session. Auto-classification if you've got 2+ hours; loose-ends batch if you want a small completed thing. Calendly webhook (Phase 1.7 Tier 2, 2–4 hours) remains available as the non-Phase-2 alternative.
