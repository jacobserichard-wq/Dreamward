...
**Small Business Edition** | Updated May 10, 2026

**8 Phases** · **55+ Tasks** · **17 Weeks** · **Phase 0–1 SHIPPED** · **Phase 1.5 SHIPPED** · **Phase 1.6 IN PROGRESS (OAuth submission paused — CASA decision)** · **Phase 1.7 SHIPPED (white-glove Tier 1)** · **Phase 2: mobile-responsive 10/10 + AI auto-classification MVP shipped**
...
- **Taxonomy gaps — telecom / utilities not in universal-core or many overlays.** Surfaced during sub-session 11 testing when AT&T was forced into "Software & SaaS" at 70% confidence. Several industries (marketplace, ecommerce, creative, etc.) have customer-facing telecom and internet expenses but no matching category. Decision: either promote `Telecom & Utilities` to universals, or add it to specific industry overlays. ~30 min commit when prioritized. Reference: sub-session 11 testing notes.
- **Settings UI not reachable from dashboard nav.** No "Settings" link anywhere in the header — customers have to type `/settings` in the URL bar. Real UX gap. ~10 min commit to add a settings link/gear icon to the header.
- **Save-categories UX is two-step and unclear.** "Add" updates local state only; "Save categories" is a separate button that persists. Customers can think they've saved when they've only added (the original cause of testing confusion in sub-session 11). Possible fixes: auto-save on add, or a clearer "unsaved changes" indicator. ~20–30 min UX commit when prioritized.
- **Default landing tab is Emails; should be Dashboard.** New users (and returning users) currently land on the Emails tab when they sign in — the noisy data-management surface — instead of the Dashboard's overview cards (Total Items, Total Amount, Status Breakdown). Dashboard is the "where am I, what's going on" view; should be the default. Likely small fix depending on how tab state is initialized (component default `useState`, URL routing, or `localStorage` last-tab memory). ~15–20 min commit.
- **`processed_items` schema telemetry** (deferred from sub-session 11's audit-recommended 7-commit plan, audit commit #5). Adds `ai_classified_at`, `ai_model`, `user_corrected_category`, optional `ai_confidence` columns to `processed_items`. Enables auditing what was classified by what/when and tracking user corrections for future learning loop. ~30 min commit including Railway migration.
- **`/api/reclassify` endpoint + dashboard button** (deferred from sub-session 11's plan, audit commit #6). Lets users (or admin) re-run classification on existing items with the updated category lists. Closes the "mixed-state dashboard" finding from commit `41e620c` where older items still show umbrella values. ~45–60 min commit.
- **Weekly reclassify sweep cron** (deferred from sub-session 11's plan, audit commit #7). Optional. Extends `app/api/cron/route.ts` to auto-reclassify items older than N days where confidence is below a threshold. ~30 min commit. Deferred pending production traffic data on classification quality.
...
**Other Phase 2 items:**
- **AI auto-classification** — MVP shipped sub-session 11 (industry-aware classification on both Gmail and CSV ingest paths + customer-facing settings UI). Three follow-up commits deferred to a future "polish" sub-session: schema telemetry, `/api/reclassify` endpoint + dashboard button, weekly reclassify cron. See Phase 2 loose-ends list above for full detail.
...
**Recommended next coding session:** With the AI auto-classification MVP shipped, **Calendly webhook** (Phase 1.7 Tier 2, 2–4 hours) becomes the primary fresh-energy candidate — turns "user visited welcome-pro" into "user actually booked," with banner copy upgrading from generic prompt to specific call time. The strong alternative is a **sub-session 12 polish batch** of the three audit-deferred AI classification commits (schema telemetry ~30 min, `/api/reclassify` endpoint + dashboard button ~45–60 min, optional weekly reclassify cron ~30 min) — together ~2 hours and the natural follow-up to sub-session 11. Smaller warm-ups still queued — Phase 1.7 Tier 2 items (**sample data parity** for the 7 missing industries and **Pro reminder cron** chasing unbooked Pro signups, ~1–2 hours each), plus three audit-deferred items from sub-session 10 (**README rewrite** ~30–60 min, **`.env.example` creation** ~20 min, **dependency hygiene sweep** ~30–90 min), plus three new UX-focused items from sub-session 11's testing (**taxonomy gaps for telecom/utilities** ~30 min, **settings link in dashboard nav** ~10 min, **save-categories UX clarity** ~20–30 min). Calendly is the headline product-trajectory move; the sub-session 12 polish batch is the alternative that closes the AI capstone fully.
