...
// loose end; a third copy now — extract to a shared helper if/when a fourth
// consumer appears).
...
export async function POST() {
  try {
    ...
    const countResult = await pool.query(
      ...
      [client.id, UMBRELLA_VALUES]
    );
    const totalBeforeReclassify = countResult.rows[0]?.n ?? 0;

    if (totalBeforeReclassify === 0) {
      return NextResponse.json({ reclassified: 0, remaining: 0, total: 0 });
    }
    ...
    const targetResult = await pool.query<TargetItem>(
      `SELECT id, vendor, amount, invoice_number, summary
       ...
       LIMIT $3`,
      [client.id, UMBRELLA_VALUES, BATCH_LIMIT]
    );
    ...
    try {
      ...
      return NextResponse.json(
        { error: "Reclassify failed mid-batch; no rows were updated." },
        { status: 500 }
      );
    } finally {
      dbClient.release();
